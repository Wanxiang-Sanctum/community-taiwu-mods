...
编译单元、检查调用方声明的入口契约、在当前宿主进程中调用入口，并把结果整理为通用运行事实。
...
- 脚本在目标宿主进程内完全信任运行；沙箱或权限分级不属于本模块承诺。
...
- 宿主选择、调用参数和其它调用方请求字段如果要暴露给脚本，应由调用方放进自己的 globals 类型。
...
运行器不会展开扫描插件目录。调用方需要把脚本入口契约程序集、明确开放给脚本的 facade 程序集，以及宿主决定暴露的运行时能力
解析为稳定 DLL 路径后传入。
...
`DynamicScriptAssemblyReferenceResolver` 是宿主侧可选使用的程序集路径工具。调用方可以用 marker type 或 `Assembly`
从已加载程序集的 `Assembly.Location` 取得引用路径；如果运行时用字节加载导致没有可用位置，调用方应按自己的部署边界构造确切
DLL 路径，并用 resolver 按 `AssemblyName.FullName` 校验该文件身份。校验后的路径再交给 `DynamicScriptReferenceOptions`。
...
{
    public JObject Arguments { get; } = (JObject)arguments.DeepClone();
...
public static class MyScript
{
    ...
    {
        string? operation = globals.Arguments.Value<string>("operation");
...
契约或发布依赖构造稳定路径后传给 runner；可用 `DynamicScriptAssemblyReferenceResolver.GetVerifiedAssemblyReferencePath`
校验显式路径的程序集身份。

## 维护约定

- shared 层只整理通用运行事实；运行结果不携带脚本正文、鉴权材料或宿主 IPC 信息。
...
- 本项目只引用 Roslyn 编译 API；Roslyn 运行时 DLL 由 `Wanxiang.Prelude` 的前后端入口项目携带并预加载。
  接入 Mod 不在自己的前后端入口里为本项目重复复制 Roslyn DLL。
- 游戏进程内新增 Roslyn 承载点时，由新增承载点自己的项目文件显式声明版本和运行时提供方式；不要让
  `GameRuntime` 项目自动继承统一 Roslyn 版本约束。
