...
运行器不会展开扫描插件目录。调用方需要把脚本入口契约程序集和明确开放给脚本的 facade 程序集解析为稳定 DLL 路径后传入。
本仓库的前端适配模块 `Wanxiang.Taiwu.DynamicScripting.Frontend` 在同一显式引用模型下固定加入 UniTask 编译引用。
...
契约或宿主固定能力解析出稳定路径后传给 runner；可用 `DynamicScriptAssemblyReferenceResolver` 解析按完整程序集身份声明的引用。
