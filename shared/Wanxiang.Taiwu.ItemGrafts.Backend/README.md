...
并把后端宿主事件转发给前端会话。后端持有需要释放的注册资源、Harmony 观察点和宿主会话统计，因此提供
`Uninstall()`。

本组件的作用域限定为同一 Mod 内的前后端协作。Harmony 观察点、宿主会话统计、后端事件流和跨端消息编码都是内部实现；
...
`BackendInventoryGrafts.Uninstall()` 会移除 ItemGrafts 的后端处理器、卸载 Harmony 观察点、清空宿主会话统计，
并停止向前端转发事件。
