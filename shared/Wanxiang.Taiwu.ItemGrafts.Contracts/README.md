...
“嫁接”使用一个游戏后端真实存在的非堆叠 `ItemKey` 作为宿主，在前端行囊入口呈现替换后的名称、描述、图标、
品级和操作。前端负责建立和结束嫁接会话，后端负责观察宿主存在性、角色行囊位置和宿主数据变化。
Contracts 是这些稳定模型和跨端内部协议的所有者；创建宿主、菜单操作、通知、Harmony 观察和会话统计分别由端侧项目负责。
...
ItemGrafts RPC 内容和后端补丁标识由前后端实现程序集管理。前端通过 `InventoryGrafts.Install(plugin)` 绑定本 mod，
