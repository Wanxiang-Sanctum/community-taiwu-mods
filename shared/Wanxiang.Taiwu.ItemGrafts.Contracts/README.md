...
“嫁接”使用一个游戏后端真实存在的非堆叠 `ItemKey` 作为宿主，在前端支持的展示入口呈现替换后的名称、描述、图标、
品级和操作。前端负责建立和结束嫁接会话，并决定如何把这些契约应用到可视化和操作入口；后端负责观察宿主存在性、
角色行囊位置和宿主数据变化。Contracts 是稳定模型和跨端内部协议的所有者；创建宿主、可视化、菜单、通知、观察和
会话统计分别由端侧项目负责。
