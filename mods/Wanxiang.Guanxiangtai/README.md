...
观象台给熟悉 agent 与 MCP 的 Mod 制作者使用，让 agent 把当前游戏运行时纳入自己的工作循环。agent 经由 MCP server
请求动态执行使用者信任的 C# 编译单元，按任务需要读取、判断或改动游戏前端、后端进程中的运行时状态。

观象台随 Mod 分发一个 HTTP MCP server。游戏前端会确保该 server 进程启动，维护者也可以手动运行包内可执行文件。server
使用可见终端窗口运行，游戏退出不会自动关闭。太吾生命周期工具和状态检测围绕这条运行时桥梁提供；agent 只连接 MCP server，
由 server 转给游戏侧能力。
