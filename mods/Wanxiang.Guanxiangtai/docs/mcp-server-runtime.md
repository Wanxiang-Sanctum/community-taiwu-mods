...
运行态入口文件 `.guanxiangtai-runtime/mcp-server-endpoints.json` 只服务同一 Mod 内的前端启动器和 MCP server 外部入口协调。它不是 MCP
...
- 前端插件：随游戏前端加载，负责检查 MCP server 入口并按需提交脱离启动请求，同时发布前端 IPC endpoint。
...
- 无参数启动：进入常驻 HTTP MCP server，使用可见控制台窗口，且不随游戏进程退出。
- `--launch-detached`：前端插件使用的自举入口。该短命进程通过 Win32 进程创建接口启动不带参数的常驻 server 实例后退出。
  常驻实例以当前桌面会话的 Explorer 为父进程，并使用独立控制台窗口。

自举入口只执行一次进程创建。创建失败时，短命进程把错误写入标准错误并以非零退出码返回；前端插件记录该失败，不在启动机制内重试。
自动拉起路径只让短命自举进程成为太吾游戏进程的子进程；常驻 server 不作为太吾游戏进程的常驻子进程，从进程关系上避免影响 Steam
的游戏运行中判定。
...
bearer token 优先从常驻 server 自身进程环境变量 `WANXIANG_GUANXIANGTAI_MCP_TOKEN` 读取；显式设置时至少需要 16 个字符。
手动从终端启动时，server 继承当前终端环境；通过 `--launch-detached` 自动拉起时，常驻 server 使用桌面用户环境，游戏进程的临时环境只属于
短命自举进程，不作为 token 传递渠道。环境变量缺失或为空时，server 会生成随机 token，并只打印在 server 控制台窗口中。
token 不从本地配置覆盖，不写入运行态文件。server 通过 ASP.NET Core authentication/authorization 管线保护 `/mcp`；
有效 MCP 请求必须带 `Authorization: Bearer <token>`。
...
开发态直接运行 MCP server 项目时，运行目录位于 server 输出目录下的 `.guanxiangtai-runtime/`。运行目录由 Mod 目录或
server 输出目录派生，不通过启动参数覆盖。观象台不把这类运行态入口信息写入用户本地应用数据目录。
...
文件由 MCP server 写入。前端启动器读取它判断是否需要提交启动请求；已有 live server 时，新启动的 server 也会读取它并输出现有入口后退出。
当前不设置独立版本字段；时间字段使用带偏移的 ISO 8601 值，字段名只表达事件含义，不额外写 `Utc`。
...
前端插件启动 server 前也会先读取运行态入口文件，避免正常路径下重复打开控制台窗口。
...
观象台的标准接入路径是手动 HTTP MCP 注册；面向使用者的步骤见 [README.md](../README.md#agent-接入)。本设计只固定可连接信息来源：

- MCP client 使用 HTTP URL 和 bearer token 连接。
- 固定端口来自 `Guanxiangtai.Local.json`，随机端口来自 server 控制台窗口输出或 `.guanxiangtai-runtime/mcp-server-endpoints.json`。
...
- `guanxiangtai_stop_taiwu`：MCP server 按 `method` 停止太吾，默认 `force`。`force` 是 OS 级强杀：先杀本机太吾前端进程树，
  ...
  被前端 IPC 接受，工具返回 `kind=requestQuitFailed`，不进入进程消失等待。停止工具不会停止 MCP server。
...
OS 进程存活性，避免把内部 IPC 入口文件或 PID 观察误包装成游戏侧可用事实。
...
运行态入口文件不保存 token、权限决策或脚本内容，也不暴露前端、后端内部 IPC endpoint。内部 IPC endpoint 只写入
`ipc-endpoints.json`，供 MCP server 路由使用；脚本请求内容只随工具调用进入目标侧，不作为运行态文件持久化。
