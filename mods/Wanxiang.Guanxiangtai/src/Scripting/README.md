...
- 目标侧插件部署目录，由前端或后端从游戏提供的观象台 Mod 目录派生，例如 `Plugins/Frontend` 或
  `Plugins/Backend`。
...
运行器编译完整 C# 编译单元。脚本入口只接收 `GuanxiangtaiScriptGlobals`；脚本需要的 `using`、namespace、类型
和返回值都由脚本自己声明。
...
入口类型可以放在脚本自己的 namespace 下，但简单类型名必须是 `GuanxiangtaiScript`，且必须是
`public static` 非泛型 class。入口方法可以是 `Execute` 或 `ExecuteAsync`，必须是 public static 方法，
参数必须是 `Wanxiang.Guanxiangtai.Scripting.GuanxiangtaiScriptGlobals`，可以通过
`using Wanxiang.Guanxiangtai.Scripting;` 简写为 `GuanxiangtaiScriptGlobals`。同步返回值、`Task` 和
`Task<T>` 都会解析为入口返回值；推荐需要异步时让入口返回 `Task<object?>`。UniTask 适合作为入口内部 await 的
