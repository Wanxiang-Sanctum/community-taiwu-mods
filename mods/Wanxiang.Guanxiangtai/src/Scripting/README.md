...
`src/Scripting/` 是观象台前端插件和后端插件共用的脚本入口适配层。它声明观象台自己的脚本入口契约，并把实际编译、
引用解析、入口调用和返回值整理委托给 `shared/Wanxiang.Taiwu.DynamicScripting`。编译 warning 不进入 Agent 可见工具返回。

脚本可见的 `GuanxiangtaiScriptGlobals` 定义在相邻的 `src/Scripting.Contracts/` 项目中，并作为独立 DLL
部署。`src/Scripting/` 拥有入口类型约定、运行器适配、IPC 结果映射和脚本契约 DLL 的引用路径声明；
前端和后端宿主从这里取得契约引用路径，再按本侧部署边界补充自己的脚本引用选项。

跨进程请求与响应归 `src/Ipc/`，MCP 工具路由归 `src/McpServer/`，可访问的游戏 API、运行状态和线程边界归目标侧
前端或后端模块。本模块只在这些边界之间声明观象台脚本入口契约。
...
- 宿主侧显式传给脚本运行器的程序集引用路径。观象台宿主会加入 `Wanxiang.Guanxiangtai.Scripting.Contracts.dll`，
  这是脚本入口参数类型所在的窄契约程序集。
- 前端脚本宿主会加入 `Wanxiang.Prelude` 前端运行时提供的 `UniTask.dll`，使脚本可以显式
  `using Cysharp.Threading.Tasks;` 并在 `Task<object?>` 入口内部 await UniTask API。
...
显式程序集引用路径是目标侧宿主配置，不是 MCP 工具参数，也不是预置命名空间。`src/Scripting/` 按当前插件目录声明脚本
契约程序集；后端宿主只使用该契约引用，前端宿主再按观象台对 `Wanxiang.Prelude` 的发布依赖声明 UniTask 编译引用。
脚本仍然自己声明需要的 `using`，入口异步契约仍是 `Task` 或 `Task<T>`。
...
运行器编译完整 C# 编译单元。脚本入口只接收 `GuanxiangtaiScriptGlobals`，入口类型完整名必须是
`Wanxiang.Guanxiangtai.Scripting.GuanxiangtaiScript`。脚本可以在入口外自行声明需要的 `using`、辅助类型和返回值结构。
...
public static class GuanxiangtaiScript
{
    public static Task<object?> ExecuteAsync(GuanxiangtaiScriptGlobals globals)
    {
...
public static 方法，参数必须是 `Wanxiang.Guanxiangtai.Scripting.GuanxiangtaiScriptGlobals`。同步返回值、
`Task` 和 `Task<T>` 都会解析为入口返回值；推荐需要异步时让入口返回 `Task<object?>`。UniTask 适合作为入口内部
