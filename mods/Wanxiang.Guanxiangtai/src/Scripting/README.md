...
部署。`src/Scripting/` 只负责运行器适配、IPC 结果映射和入口契约声明；引用路径由目标侧宿主通过
...
- 宿主侧显式传给脚本运行器的程序集引用路径。观象台宿主会加入 `Wanxiang.Guanxiangtai.Scripting.Contracts.dll`，
  这是脚本入口参数类型所在的窄契约程序集。
- 前端宿主按显式 feature 把运行时已加载的 UniTask 程序集解析为编译引用，
...
显式程序集引用路径是目标侧宿主配置，不是 MCP 工具参数，也不是预置命名空间。宿主用
`DynamicScriptAssemblyReferenceResolver` 解析必需的脚本契约程序集；前端能力引用由
`FrontendScriptReferencePaths` 按显式 feature 解析。脚本仍然自己声明需要的 `using`，入口异步契约仍是 `Task`
或 `Task<T>`。
