...
这个模块不直接暴露游戏进程，也不实现前端或后端插件能力。后续 tools 应继续由 MCP server 对 agent 暴露，再通过内部 IPC/bridge
访问游戏侧模块。
...
- 不维护 MCP client 配置。
- 不创建或假设默认 agent 工作区。
- 不把前端或后端游戏进程登记为 agent 可连接入口。
- 不保存 token、agent 会话、脚本内容、调试会话或游戏进程 IPC 地址。

包内运行时，server 会从 `Processes/Wanxiang.Guanxiangtai.McpServer/` 反推 Mod 目录。开发态直接运行项目时，server 输出目录作为运行目录 owner。
