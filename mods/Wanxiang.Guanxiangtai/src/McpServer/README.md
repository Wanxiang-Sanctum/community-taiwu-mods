...
- Agent 的可连接入口是 MCP server HTTP endpoint；使用者把该 URL 和 bearer token 登记到自己的 MCP client。
- 前端和后端游戏进程只由本 server 在模块内部调用，工具返回体只携带工具契约字段。
- 生命周期工具的参数只选择操作策略；前端/后端 side 和等待窗口由本模块按完整太吾运行时固定处理。
- 本模块自身入口登记只记录 MCP server HTTP 地址和进程信息；token 与脚本内容不写入这份持久状态。
