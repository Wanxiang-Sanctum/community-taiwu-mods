...
| `Wanxiang.Guanxiangtai/` | 熟悉 agent 与 MCP 使用的 Mod 制作者接入当前游戏运行时的 MCP 入口；通过动态 C# 脚本把运行时状态接入 agent 工作循环。                                  | `Wanxiang.Guanxiangtai/README.md` | `Wanxiang.Guanxiangtai/DEVELOPMENT.md` |
| `Wanxiang.Prelude/`      | 提供共享运行时和插件依赖加载规则的前置 Mod。                                                                                                         | `Wanxiang.Prelude/README.md`      | `Wanxiang.Prelude/DEVELOPMENT.md`      |
| `Wanxiang.Xiangshu/`     | 把本地 CLI Agent 接入当前运行中的太吾绘卷，通过动态脚本尝试完成玩家提出的查找、判断和改动目标；当前支持 Codex CLI 和 CodeBuddy Code。                | `Wanxiang.Xiangshu/README.md`     | `Wanxiang.Xiangshu/DEVELOPMENT.md`     |
...
DLL。shared 的部署动作只由标准 `ProjectReference` 决定，不再手写
...
插件入口项目的其它额外 DLL 依赖有两种部署动作。`Include` 只写 DLL 文件名；对应 DLL 必须先通过
