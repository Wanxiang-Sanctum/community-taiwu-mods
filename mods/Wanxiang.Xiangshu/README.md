...
- 太吾 Mod 用户配置能够记录相枢内部 Agent 类型、CLI 入口和工作目录。

可用对话窗口、本机 CLI 会话调用、游戏状态修改和外部业务服务对接都不在当前阶段内。相枢内部 Agent
会话后续由前端插件内部管理；MCP server 不承载主对话协议，只作为注册给本机 Agent 的相枢工具服务。
...
前端插件会把一个诊断命令注册到游戏原生热键系统的地图热键分组中。默认热键是 `Ctrl+F10`；该命令
当前在系统设置里暂时复用游戏内 `Mod` 文本显示，键位本身走游戏原生热键保存和冲突检测。按下热键后，
前端会读取当前 Agent 配置，等待相枢 MCP sidecar endpoint 注册，然后启动所选 CLI：
