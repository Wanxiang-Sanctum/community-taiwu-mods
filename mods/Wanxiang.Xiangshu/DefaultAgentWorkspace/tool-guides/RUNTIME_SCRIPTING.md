...
用本文件判断相枢 Mod 脚本工具的运行时语义和入口契约。你从玩家目标判断是否需要脚本能力。使用前先确认当前
可用工具中存在脚本运行工具；工具名称、参数和副作用以当前可用工具说明为准。

## 运行工具边界

相枢 Mod 的脚本工具通常是 `xiangshu_run_csharp_script`：在游戏前端或后端进程中，通过相枢插件运行完全受信的
C# 脚本。当前可用工具说明是工具是否存在、工具名称、参数闭集和返回形态的即时契约。

脚本不是沙箱。会修改状态的代码会直接影响当前游戏进程。这里的脚本面向已加载游戏进程中的运行时对象；
这里的当前运行世界事实都指这些运行时对象；静态存档文件读写不属于默认脚本路径。目标、影响和玩家意图不清楚时，
先运行最小只读查询。仍辨不清落点时，按 `AGENTS.md` 的当前请求处理和愿望回应规则收束。脚本侧只负责这些边界：
API、类型、目标侧或脚本形态属于脚本路径内部选择，优先用只读探测、领域指引和游戏知识入口自行收窄。

只在脚本侧语义中使用这些规则。工具注册、传输、可用性和相邻输出工具由相枢 Mod 运行时、`AGENTS.md`
输出边界和当前可用工具决定。
...
脚本在目标侧游戏进程内编译并执行，不在独立 sidecar 进程里执行。目标侧影响可引用程序集、可访问的游戏 API、
线程边界和可见运行状态。

- `frontend`：前端侧，项目目标框架是 `netstandard2.1`。用于前端 UI、聊天窗口、热键、Unity 对象、当前
  界面和前端运行态。若问题只需要玩家当前看见的画面，先按 `PLAYER_VIEW.md` 取得玩家可见证据；脚本用于
  定位、读取目标 UI 状态或补充验证。
- `backend`：后端侧，项目目标框架是 `net8.0`。用于后端游戏运行数据、世界/角色/物品/地图等后端
  API 和后端运行态。
...
- `current`：默认值，在目标插件的请求处理线程调用入口。用于纯计算、程序集/类型探测和格式整理等不依赖游戏运行态的
  脚本。
...
- `Side`：当前目标侧。
- `Arguments`：来自工具必填参数 `arguments` 的 JSON 对象；没有参数时也传 `{}`。按 `JObject` 读取属性、数组和值。
...
写脚本时用这些常见入口锚点取得游戏领域上下文；不要把它们当作完整 API 清单或查询体预设。先用类型和方法
层级入口缩小范围；返回对象形态按目标侧游戏进程的当前结果确认。
...
读取当前运行世界或界面运行事实时，优先用这些直接路线减少无目标反射。玩家可见画面事实归 `PLAYER_VIEW.md`；
这些锚点只补充权威状态、目标对象身份和定位辅助。
