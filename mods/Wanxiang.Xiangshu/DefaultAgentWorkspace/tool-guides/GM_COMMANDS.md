...
当玩家明确询问 GM、调试面板、作弊/测试命令、隐藏 GM 选项，或想借游戏内预置调试能力定位或改动当前运行世界时，
读取本文件。普通游戏机制查询仍先走 `GAME_KNOWLEDGE.md`；当前事件、事件选项和事件脚本结构仍先走
`EVENTS.md`；实际脚本入口、目标侧、线程规则和写操作边界仍以 `RUNTIME_SCRIPTING.md` 为准。

本文件只提供定位路线、边界和少量高频锚点。GM 能力大多是开发调试入口，不是稳定玩家玩法接口；完整清单和具体实现
以当前游戏进程中加载的程序集、配置和事件包为准。
...
需要确认玩家是否能看见或操作 GM 面板时，先按 `PLAYER_VIEW.md` 取得玩家可见证据；需要枚举或解释命令时，
在 `frontend` 目标侧读取 GM 类型和本地化；需要改后端世界状态时，优先确认当前世界事实，再决定是否调用 GM
方法或改用更直接的后端域 API。
...
4. 如果可见条件是永假表达式，把结论限定为“配置存在但不是当前正常显示入口”。普通问答只解释结构事实；
   执行等价状态改动属于写操作，需玩家明确调试、作弊、跳过或修改目标，并核对当前事件、奇遇或地图上下文。
...
可接续：命令行名称可用于解释 GM 控制台用法；方法名和类型名可用于继续读取实现或在玩家明确要求时执行对应写操作。
...
玩家问题明确涉及开关状态、面板页或对象块时，再按 `GMMemberAttribute` 扩展枚举范围。
...
- 优先把 GM 能力当作定位和理解游戏内部调试路径的资料源；能用稳定后端域 API 完成的当前世界改动，通常直接用后端
  域 API 更可控。
...
- 玩家只问“有没有 GM 指令、它们是什么”时，停在只读枚举和解释。玩家明确要求使用 GM、作弊、跳过、生成、修改或
  调试状态时，再按 `RUNTIME_SCRIPTING.md` 的写操作规则执行。
