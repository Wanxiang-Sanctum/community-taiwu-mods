...
本地化、模板/显示辅助、百晓册内容、目标侧或 namespace 时，读取本文件。当前角色、地点、背包、地图、界面状态等运行时事实按
当前输入或运行工具读取；玩家当前可见画面和可见结果按 `PLAYER_VIEW.md` 处理。

基础相枢身份和口吻由 `AGENTS.md` 处理；更细的人设校准由 `persona/README.md` 处理，静态世界观背景由
`lore/README.md` 路由。需要游戏内官方资料、配置表、本地化文本、模板/显示辅助或百晓册文本时，从这里选择
运行时知识入口。
...
2. 当前运行世界、界面、角色或物品实例的权威状态会影响回答时，按 `RUNTIME_SCRIPTING.md` 选择前端或后端读取当前事实。
3. 需要稳定游戏知识时，优先查 `Config.*` 配置表、本地化、模板显示辅助和百晓册数据结构。
4. 直接入口失败、类型或方法不确定，或游戏版本差异导致编译失败时，做小范围反射；反射限定到目标 namespace
...
- `EventEditor`、`AdventureEditor`、`AiEditor`、`DisplayConfig` 等偏编辑器、调试或旧系统命名空间，只在
  当前目标涉及编辑器、工具面板、旧界面或需要借这些入口定位时再查。当前目标涉及 GM 面板、调试命令、隐藏
  GM 选项，或需要借 GM/调试入口定位状态改动实现时，先读取 `GM_COMMANDS.md`，再按需要回到本文件补充配置、
  本地化和 namespace 定位。

## 配置与本地化

配置表是机制、模板、基础数值和枚举关系的首选入口。稳定描述停在表类型和方法层级：

- `Config.*`：配置表类型。按模板 id、refName、名称、类型或条件查询时，先选定对应表类型，再由当前脚本读取必要事实。
- `ConfigData` 系列配置基类：用于理解配置表枚举、按键读取和 refName 转换的共同形态；访问形态由当前表类型确认。
- `GetAllKeys()`、`GetItemId(refName)`、`GetRefName(templateId)`：常见配置查询方法。
- `LocalStringManager.Get(...)`、`LocalStringManager.GetFormat(...)`：读取本地化文本。
- `GameDataExtensions.LocalStringManagerHelper`：需要判断当前语言或调用本地化辅助时的类型线索；所需入口由当前脚本确认。

物品、功法、角色显示名和界面展示文本常有专用辅助类型。先用配置表和已有显示辅助；找不到明确入口时，
再按目标类型名做小范围探测。

`LanguageKey.*.Tr()` 和 `LanguageKey.*.TrFormat(...)` 是前端代码里常见的本地化快捷写法。返回文本可能带
颜色、超链接或富文本标签；回答玩家时保留含义即可，颜色、超链接和富文本标签只在定位界面文本或调试显示链路时说明。

## 模板与显示辅助

常见配置表和显示辅助能直接回答大量“这是什么、如何显示、有哪些机制事实”的问题：

- 功法、技艺、门派、人物特性、建筑、地区、资源和状态通常先查对应 `Config.*` 表类型。玩家问当前事件窗口、
...
- 角色经历模板先用 `Config.LifeRecord`。经历当前事实、分页读取、按经历条件查找角色、梦回经历和好感可见性边界由
  `LIFE_RECORDS.md` 负责；这里只取模板和命名空间定位。
- 物品 `itemType` 和 `templateId` 应来自当前 `ItemKey`、配置项、refName 转换或运行时查询；需要数值枚举时，
  以配置表和运行时事实为准。
- 前端显示可参考 `ItemUtils.GetItemColorName(...)` 和当前运行时可用的颜色/品级显示辅助；回答事实问题时，
  优先返回名称、品级和说明；颜色标记只作为显示辅助。

当前运行世界里的具体物品、角色、地图块、建筑和功法实例不是模板事实。需要回答“我现在有多少、在哪、是谁、装备了
...
- `EncyclopediaDataManager`：节点树入口类型。
- `EncyclopediaContent`：从当前语言的 `EncyclopediaAssets/EncyclopediaContent.tsv` 读取正文、标题、
  key、超链接和插入引用的入口类型。
- `EncyclopediaReference`：从当前语言的 `EncyclopediaAssets/EncyclopediaReference.tsv` 读取表格、
  图片、超链接和引用参数的入口类型。
- `EncyclopediaDataProcessor.GetTable(tableName)`：读取百晓册引用的表格数据。
- `NodeData`：百晓册节点类型；标题、正文、父子关系和配置关联按当前查询读取。
...
- `ViewEncyclopediaPanel.OpenLink(...)`
- `UIManager` 的界面显示方法；实例入口由当前前端运行时确认。

普通问答直接读取数据结构并返回简洁结果。

## 查询结果

查询时只取检索路线和结果边界；不要从本文件生成脚本写法。优先返回少量结构化事实，再由相枢组织成玩家可读答复。

- 百晓册查询返回候选节点、key、标题、正文摘要、表格名或匹配来源即可；结果范围限制在当前问题需要的资料。
- 配置查询先确定表名、refName、模板 id 或目标类型，再读取必要事实；结果范围限制在当前问题需要的配置项。
...
- 查当前运行世界、角色、地图、物品实例、后端域或会改变世界状态的操作：优先 `backend`，必要时再用前端显示辅助
  补名称。
- 查纯配置表、本地化或模板资料：两侧都可能可用；优先选离当前问题最近的一侧，失败后再换侧。
- 查物品模板名称、描述、品级和子类：优先用 `ItemTemplateHelper`，再查对应 `Config.*` 表。
...
- 探测代码保持只读。
- 反射输出先整理为候选类型名或方法签名摘要，再转成玩家可理解的答复。
