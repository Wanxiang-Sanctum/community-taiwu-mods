...
当玩家询问游戏机制、术语、物品/功法/建筑/门派等资料，或使用运行能力前需要先解析游戏内名称、说明、配置、
本地化、模板/显示辅助和百晓册内容时，读取本文件。当前角色、地点、背包、地图、界面状态等动态事实仍按
本轮输入或运行工具读取，不从静态知识推断。

普通相枢人设、口吻和世界观背景先走 `context/README.md`；只有需要游戏内官方资料、配置表、本地化文本、
模板/显示辅助或百晓册文本时，才使用这里的运行时知识入口。

## 优先检索顺序

先走直接数据源，最后才反射：

1. 已经在本轮输入或静态 `context/` 中明确的内容，直接使用。
2. 当前存档、界面、角色或物品实例会影响回答时，按 `SCRIPTING.md` 选择前端或后端读取当前事实。
...
什么”时，按 `SCRIPTING.md` 走后端域读取当前状态，再用配置和显示辅助补名称。
