...
| `Claude` | Claude Code | `claude` | `CLAUDE.md`、`.claude/skills/` | stream-json `type = "system"`、`subtype = "init"` 事件的 `session_id` | result event 中的 `structured_output.reply` |
| `CodeBuddy` | CodeBuddy Code | `codebuddy` | `AGENTS.md`、`.agents/skills/` | stream-json `type = "system"`、`subtype = "init"` 事件的 `session_id` | result event 中的 `structured_output.reply` |
...
当前轮次输入 JSON 通过 stdin 传入。前端会话从 Codex JSONL 的 `type = "thread.started"` 事件捕获 `thread_id`
作为 `agentSessionId`，并只读取 `--output-last-message` 指定临时文件中的最终答复。
...
序列化出的 compact JSON。Claude Code 与 CodeBuddy Code 适配器都只从 result event 的 `structured_output.reply`
提取最终答复。
