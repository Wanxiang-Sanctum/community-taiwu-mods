...
本文维护相枢 Mod 前端可启动的本地 CLI Agent 适配器清单、配置入口和命令契约。`agent-chat.md`
只描述游戏内对话链路的稳定模型；新增或调整具体 Agent 时，优先更新本文、`Config.Lua`、`src/Frontend/Agent/`
和 `src/Frontend/Agent/Cli/`。
...
默认 Agent 工作区当前通过 `AGENTS.md` 和 `.agents/skills/` 服务所有内置适配器。

配置读取只认 `AgentCliAdapter` 设置 key；当前下拉值是 `Codex = 0`、`CodeBuddy = 1`。
...
## Print mode + stream-json 命令族

CodeBuddy Code 适配器复用通用 print mode + stream-json 命令族，并通过 `--dangerously-skip-permissions` 进入完全信任式
非交互模式。主 Agent 工作区来自进程工作目录，因此启动进程时设置 `WorkingDirectory = AgentWorkingDirectory`：
...
会截断多行 prompt，因此该命令族把当前轮次输入 JSON 写入 stdin，不把 `<turn-input-json>` 放在命令行末尾。
`--json-schema` 是当前命令族使用的 CLI 参数契约，因此适配器在该边界传递由同一 schema 文档
序列化出的 compact JSON。最终答复主来源是 result event 的
`structured_output.reply`；如果该字段缺失，且 result event 不是错误结果，适配器只把 result event 的
`result` 文本作为同一 schema 文档再解析一次。

后续调用使用捕获到的 `session_id` 恢复同一个 CodeBuddy Code 会话：
...
如果需要让 CodeBuddy Code 访问主 Agent 工作区之外的目录，再额外使用 `--add-dir <path>`；主 Agent 工作区仍由进程工作目录提供。
...
- `Config.Lua` 的 `AgentCliAdapter` 选项。
- `AgentAdapter` 枚举和 `AgentAdapterCatalog` 中的当前下拉值、默认命令和持久化 adapter key。
- `Agent/Cli/AgentCliAdapters` 映射和具体 `IAgentCliAdapter` 实现；注册表会在运行时校验它覆盖 `AgentAdapterCatalog`。
- 本文的当前适配器表和对应命令契约。
