...
本文面向维护相枢默认工作区的源码维护者，不是运行时 Agent 指令。源码中的 `DefaultAgentWorkspace/`
是默认工作区内容源，组包后输出为包内同名目录；包内工作区以随包发布的文件作为完整输入。

维护者文档可以说明来源、放置规则、更新边界和为什么这样组织；运行时工作区文件应直接服务 Agent 当前回答、
工具选择和脚本落地。维护本文时，不把维护理由、来源审计或“应该如何维护文档”的话术复制到
`DefaultAgentWorkspace/` 的运行时指引里。
...
可见性，包括本 Mod 的愿望回应前提：太吾传人将念头递入药钵，相枢便不得不回应；目标含混、影响未明、
目标矛盾、代价过高或贪求过甚时，偏转应由行为、执行路径、条件、波及范围或结果承载；偏转本身不主动向
太吾传人揭示。
...
工作记录。
完整协议说明保留在相枢源码中，完整 API 清单和不稳定枚举表保留在游戏侧源码
或组织内部游戏观察快照中。随默认工作区发布的初始运行经验应沉淀为工具指引、读取路由、领域上下文、参考片段
或稳定资产；`.xiangshu-notes/` 只记录本机运行后产生的草稿和本地经验。
...
下表列出组织内部游戏观察快照中的核对路径，路径相对
[`taiwu-modkit`](https://github.com/Wanxiang-Sanctum/taiwu-modkit) 仓库根目录。这些路径对应太吾游戏文本、配置和
游戏侧源码观察结果；资料主题对应太吾游戏本体。
