...
本文面向源码仓库维护。源码中的 `DefaultAgentWorkspace/` 是默认工作区内容源，组包后输出为包内同名目录；
包内工作区以随包发布的文件作为完整输入。

本文只记录默认工作区资料如何回溯到游戏观察快照。相枢仓库拥有默认工作区内容和放置规则；
[`taiwu-modkit`](https://github.com/Wanxiang-Sanctum/taiwu-modkit) 拥有游戏观察快照的生成工具和快照更新方式。快照正文、
完整 API 清单和工具配置保留在所属仓库与对应源码模块中。

`DefaultAgentWorkspace/lore/` 中的世界观资料派生自本工作区内
...
4. 涉及脚本工具、运行时目标侧、配置/本地化/模板辅助或百晓册入口时，写入
   `DefaultAgentWorkspace/tool-guides/`；保留检索路线和边界，具体任务脚本留给运行时按本轮目标生成。
