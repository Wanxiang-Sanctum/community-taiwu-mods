...
- `src/McpServer/` 拥有 MCP 工具语义：选择目标 endpoint、转发 IPC 请求，并把内部响应整理成 Agent 可读
  JSON。
- `src/Scripting.Contracts/` 提供脚本可见的窄契约程序集；`src/Scripting/` 声明入口类型约定、运行器适配和响应映射，
  并复用 shared 动态脚本运行核心。
- `src/Frontend/Ipc/` 暴露前端侧 IPC endpoint，把脚本执行、中间答复和玩家视图捕获请求接入前端模块。
- `shared/Wanxiang.Taiwu.DynamicScripting.Frontend` 提供前端侧脚本宿主适配，包括入口主线程分派和前端脚本引用选项创建；
  该适配固定加入 UniTask 编译引用。
...
扩展；入口契约和目标侧接入说明由 `src/Scripting/README.md` 维护，通用编译和临时程序集依赖解析归 shared
动态脚本运行核心，前端脚本引用选项创建归 `shared/Wanxiang.Taiwu.DynamicScripting.Frontend`。
