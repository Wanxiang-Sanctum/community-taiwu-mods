...
- `working`、`failed`、`ended` 等内部状态驱动会话推进、发送按钮状态和必要的相枢说明。
...
`AgentWorkspace/CLAUDE.md` 负责让 Claude Code 转向同目录的 `AGENTS.md`，让默认人设保持单一来源。
...
所有权属于相枢前端插件和 MCP server。默认 `AGENTS.md` 明确要求 Agent 不创建、修改、删除或整理
`XiangshuRuntime/`。具体文件格式留给前端实现和设计文档。
...
这些写入点集中在 `XiangshuRuntime/` 下，让 Agent 工作区根目录保持稳定，也让 Agent 更容易识别相枢
运行数据的所有权。
...
- IPC endpoint manifest 写入 `AgentWorkingDirectory/XiangshuRuntime/ipc-endpoints.json`。manifest 承担
  发现职责；endpoint 监听由插件生命周期维护，MCP 诊断工具当前用 ping 测试链路。
...
- 前端把当前会话选择、可见聊天消息、消息计数器和外部 Agent 会话 id 写入
  `XiangshuRuntime/ChatSessions/`；重启后在适配器一致时恢复当前聊天窗口记录并继续复用外部会话 id。
...
底纹和滚动柄资源。界面阶段到此收口：后续维护聚焦布局、焦点、滚动和输入等真实交互问题；
运行状态、工具日志和脚本控制归内部链路或诊断入口。

MCP server 当前暴露工具链诊断、IPC ping 和中间答复工具。下一阶段的脚本能力按前端/后端各自 IPC
边界分别暴露；主对话入口仍归前端投递会话。
...
- `failed`：如果需要让玩家知道失败，前端会话追加一条 `origin = "session"` 的相枢固定文本说明；原始异常、
  退出码和标准错误进入游戏日志。
- `ended`：玩家继续输入时，前端优先恢复内部会话；恢复失败时追加一条相枢文本说明。
...
下一阶段主线是把相枢工具链从诊断 ping 推进到脚本转发和实时运行。目标路径是：
...
- `src/Ipc/` 定义脚本运行请求、运行 id、结果、错误和实时进展事件的 contract。
- `src/McpServer/` 把 MCP 工具调用转成目标侧 IPC 请求，并把返回值整理为 Agent 可读的工具结果。
- `src/Frontend/Ipc/` 可以暴露前端侧脚本执行能力，负责前端进程可访问的游戏 API、界面上下文和前端运行状态。
- `src/Backend/` 可以暴露后端侧脚本执行能力，负责后端进程可访问的游戏 API 和后端运行状态。
- 会修改游戏状态的脚本能力落在实际承载该 API 和线程边界的前端或后端插件中。
- `src/Frontend/Chat/` 承担玩家可见对话；需要显示的实时进展复用相枢消息或既有中间答复通道。

第一版优先闭环“提交脚本、执行、返回结果/错误/耗时”。实时运行再在同一运行 id 下补充进展事件或中间
答复。具体实现顺序记录在文末“后续实现顺序”。
...
后续迭代优先推进 IPC 脚本转发与实时运行。建议顺序是：

- 先在 `src/Ipc/` 固化脚本运行 contract，包括目标侧、运行 id、输入、结果、错误和进展事件。
- 再分别接入 `src/Frontend/Ipc/` 与 `src/Backend/` 的脚本执行入口，让每侧承载自己能访问的游戏 API
  和线程边界。
- 然后让 `src/McpServer/` 把 MCP 工具调用路由到目标侧 IPC，并把结果整理为 Agent 可读的工具返回。
- 最后把玩家需要看到的结果或进展映射回既有相枢消息。

## 本阶段边界

- 玩家在 Agent 工作期间通过忙碌态发送按钮追加“且慢”、取消当前轮，并触发下一轮重投递。
- MCP 中间答复工具向当前本地聊天会话追加相枢消息。
- 会修改游戏状态的 MCP 工具留到脚本运行通道中实现，并按前端/后端实际 API 边界归属。
