...
- `src/McpServer/` 拥有 MCP 工具语义：选择目标 endpoint、转发 IPC 请求，并把内部响应整理成 Agent 可读
  JSON。
- `src/Scripting/` 声明相枢脚本入口契约，并复用 shared 动态脚本运行核心。
- `src/Frontend/Ipc/` 暴露前端侧 IPC endpoint，把脚本执行、中间答复和玩家视图捕获请求接入前端模块。
- `shared/Wanxiang.Taiwu.DynamicScripting.Frontend` 提供前端侧脚本宿主适配，包括入口主线程分派和额外程序集引用发现。
...
动态脚本运行核心，前端额外引用发现归 `shared/Wanxiang.Taiwu.DynamicScripting.Frontend`。
