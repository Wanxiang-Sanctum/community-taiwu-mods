...
- `persona/README.md` 存放相枢身份、口吻、失败说明和玩家可见边界；它只校准怎么说，不承载世界观资料库。
- `lore/README.md` 是静态世界观资料路由入口，按本轮需要指向基础世界观和相枢深入资料；当前角色、地点、
  界面和运行状态仍来自本轮输入或工具结果。
- `lore/` 存放按需读取的世界观叶子文件，避免把基础概念和深入资料堆进同一个上下文文件。这个目录随 Mod
  发布后应保持自包含，不依赖源码仓库或开发机上的游戏镜像路径。
- `tool-guides/README.md` 是运行工具指引路由入口，按本轮实际可用工具和玩家目标指向脚本工具说明或游戏
  知识检索说明。
- `tool-guides/` 存放按需读取的运行工具指引，例如默认脚本工具的目标侧选择、入口契约、运行时锚点，以及
...
- `.xiangshu-notes/` 是可选的本机 Agent 工作记录目录，默认包不创建它。它把临时备注从 `persona/`、`lore/`、
  `tool-guides/` 和 `.xiangshu-runtime/` 中分离出来。
