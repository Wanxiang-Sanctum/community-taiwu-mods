...
默认工作区的读取路由由 `AGENTS.md` 和各子目录 README 维护；源码维护时的资料来源、放置规则和快照核对路径见
`agent-context-sources.md`。运行设计文档不复制这些工作区文件的完整内容，只约束它们在对话链路中的职责：

- `AGENTS.md` 与 `CLAUDE.md` 是对应 CLI Agent 的入口文件；`AGENTS.md` 承担基础相枢身份、口吻和玩家可见边界。
- `persona/`、`lore/` 和 `tool-guides/` 分别承载更细的人设校准、静态世界观和运行工具指引。
- `.agents/skills/` 与 `.claude/skills/` 是对应 CLI Agent 自动发现的技能目录。
- `.xiangshu-notes/` 是可选本机工作记录目录，用于会话草稿和本地经验，默认包不创建它。
- `.xiangshu-runtime/` 是相枢前端插件和 MCP server 维护的运行数据目录，不属于可编辑 Agent 资产。
...
捕获 Codex `thread_id` 或 Claude Code `session_id` 后，在后续投递轮次通过 CLI resume 参数恢复同一个外部会话。
...
- `Temp/AgentCli/`：前端启动 Codex/Claude 时使用的短生命周期协议文件目录，例如
  `--output-last-message`、`--output-schema` 和 `--mcp-config` 所需文件；Claude 的临时 MCP config 会包含
  本次运行的 `Authorization: Bearer ...` header。单次调用结束后删除对应调用子目录。
...
`AGENTS.md` 负责入口指令；技能触发由对应技能目录负责，MCP 工具是否可用、参数和副作用由当前 CLI 调用中暴露的工具说明决定。
...
结构化投递输入通过 stdin 传入。前端会话从 Codex JSONL 事件里的 `thread.started.thread_id` 捕获
