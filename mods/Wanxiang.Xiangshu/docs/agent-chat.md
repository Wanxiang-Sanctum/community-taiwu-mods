...
协议字段，但界面不展示这些词，也不展示 Codex、Claude、MCP、CLI、权限、进程、标准错误、工作目录等
实现细节。
...
- 调试细节只进入游戏日志、MCP server 事件日志或可展开的诊断入口，不进入默认对话体验。
...
- 玩家消息会立即显示在运行时生成的聊天窗口中，并进入前端内存会话。
- 前端把等待期间的玩家消息合并成下一批，启动所选 CLI Agent，注册当前相枢 MCP endpoint，并把
  CLI 最终答复显示为相枢消息。
- CLI 失败时，前端只追加一条 `origin = "session"` 的相枢固定失败消息；原始异常、退出码和标准错误
  通过游戏日志记录。

当前对话窗口仍是最小可测界面，不持久化会话，不提供 MCP 快速答复工具，也不修改游戏状态。MCP server
仍只暴露用于工具链诊断和 IPC ping 的工具，不提供主对话入口。
...
日志不进入默认玩家对话体验，也不改变 Agent 的无人值守权限策略。游戏内前后端插件服从太吾的游戏日志
系统；需要解析 CLI JSON 和标准输出的调用仍应保留结构化内存捕获，聊天适配器只把摘要和错误写入游戏
日志。MCP server 是独立进程，可以继续把自己的事件日志写入 sidecar 诊断目录。
...
内部会话可以按这些核心字段理解。当前实现只保存最小运行所需的数据；`deliveryStatus`、
`contextPolicy` 和完整 `turns` 记录仍是后续扩展点：
...
下面记录当前实现使用的命令形态；恢复外部会话仍是后续扩展，不在当前调用路径中启用。
...
随后再接入 MCP 快速答复工具、可恢复外部会话，以及需要读取游戏状态的相枢工具。对话 UI 仍不需要直接
理解 Codex/Claude 的 CLI 参数，也不需要直接注册 MCP server；这些能力留在前端适配器和 MCP 工具层。
