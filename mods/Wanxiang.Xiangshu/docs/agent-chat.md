...
相枢的对话入口由前端插件负责。玩家界面呈现为太吾传人与寄身“低语的陶土药钵”中的相枢连续对话；前端负责
游戏内消息、投递状态、可见对话片段和 CLI 调用，本机 Agent 维护模型侧可恢复会话。交互以流畅度优先：前端启动 CLI
...
本机 Agent 会话仍负责更长程的模型侧上下文。MCP server 的职责是作为相枢暴露给
本机 Agent 的工具服务：前端在启动 CLI Agent 时，把当前相枢 MCP endpoint 注册给 Agent；Agent 对当前目标的
事实读取、结果验证或状态操作通过 MCP 调用相枢工具完成，具体调用时机由默认工作区和当前工具说明约束。
...
不参与请求归属判断；相枢的聊天状态和可见片段投递归前端投递会话，更长程模型上下文归本机 Agent 会话。
...
用户可以先沿用默认工作区，再手工维护自己的人设、世界观资料、工具指引、设置和 Agent 技能；运行中的
...
- `origin = "agent"` 的 assistant 消息表示上一次 CLI 最终答复；`origin = "agent-intermediate"` 表示 Agent
  通过 MCP 中间答复工具发出的玩家可见消息；`origin = "runtime"` 表示相枢运行时已经显示给玩家的说明，例如
...
相枢身份由默认工作区入口指令和游戏内显示层固定，不作为输入字段传给 CLI Agent；扩展人设资料只细化表达。前端
...
除必须重置说明外，相枢运行时写入的可见固定说明会进入上下文锚点队列。它不单独触发 CLI 调用；下一条普通玩家
...
- 上下文锚点队列：已经显示给玩家、尚未进入下一轮 `contextMessages` 切片的消息；包含玩家消息，以及等待下一条
  玩家消息一起送出的 `origin = "runtime"` 相枢运行时固定说明。“且慢”会暂停该队列。
...
答复提取兜底说明、失败说明和必须重置说明都以相枢消息进入对话流；运行边界和失败原因按日志策略处理。相枢
...
本节只记录对话链路依赖的适配器边界：前端把一个投递轮次交给本机 CLI Agent，等待最终答复、运行时兜底或协议失败；
