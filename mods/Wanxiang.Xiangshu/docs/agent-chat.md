...
CLI 的完全信任式非交互执行方式，避免游戏内对话等待权限交互，让一次投递轮次收束为一条相枢最终回应。
当前接入两个适配对象：
...
- `working`、`failed`、`ended` 等内部状态只驱动会话推进、发送按钮状态和必要的相枢说明。
...
事件日志混入聊天记录。前端当前不持久化 `pendingMessages`，因此重启后不会自动补投上次关闭时尚未完成
投递或因中断等待重投递的内存消息；若当前会话已恢复，玩家继续输入时会从恢复的外部 Agent 会话 id
继续新的轮次。
...
- 玩家消息会立即显示在运行时生成的聊天窗口中；Agent 空闲时，消息立即进入一个投递轮次。
...
投递轮次；当前 CLI Agent 正在工作时，常规发送入口切换为中断入口，不把玩家后续输入排队等待当前答复。
...
前端通过 `externalSessionId` 恢复本机 Agent 自己的会话；结构化回合输入只承载本轮玩家消息和参与者信息。
...
每个投递轮次对应一次 CLI Agent 调用。当前轮次只携带本轮待投递的玩家消息；assistant 答复和中间答复
按产出顺序追加到可见对话记录。玩家发起中断后，旧 CLI 调用的最终答复不会进入可见对话记录；中断状态
内以及下一轮建立前晚到的中间答复会被忽略。当前轮玩家消息会与“且慢”一起重新形成下一轮。后续如果需要
把前端侧故障、状态压缩说明或更严格的中间答复轮次归属交给 Agent，应使用独立的前端事实字段描述这些
事件；可见对话记录仍保持为界面层数据。

## 对话界面映射

对话界面保持单一主交互，不暴露独立的 Agent 控制台，也不把“正在输入”“正在思考”等 IM 状态插入对话流。
界面只渲染玩家和相枢已经发送出来的消息：

- 玩家消息：玩家发送后立即追加到对话流。
- 相枢消息：只有在 CLI 最终答复、MCP 中间答复工具或前端固定说明产出文本时才追加。
- `working`：不产生可见消息，但发送按钮切换为可点击的“且慢”中断入口。
- `interrupting`：玩家点击“且慢”后，按钮进入不可重复点击的止息状态，直到当前 CLI 调用退出并开始下一轮。
...
异常退出或被阻断，前端会话按 `failed` 映射成相枢文本答复。

当前 `codex exec` 与 `claude --print` 适配只有进程边界，没有回合级控制通道。玩家“且慢”不会向 CLI
发送原生中断；前端会取消当前轮、结束 CLI 进程，并把上下文重投递给下一轮。Codex app-server 的
`turn/interrupt` 和 Claude Code SDK streaming 的 `interrupt()` 属于不同 transport，只有实际改接时才进入
适配器。
...
- 玩家在 Agent 工作期间不再排队发送自由文本；忙碌态发送按钮只追加“且慢”、取消当前轮，并触发下一轮重投递。
