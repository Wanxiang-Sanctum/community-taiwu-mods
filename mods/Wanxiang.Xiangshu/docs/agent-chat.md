...
投递轮次；CLI Agent 正在生成回复时，发送入口切换为“且慢”中断入口。

玩家点击“且慢”后，前端追加一条同名玩家消息，取消当前 CLI Agent 调用，并丢弃该调用后续结果。“且慢”
进入 `pendingMessages` 后会暂停投递；玩家发出下一条普通消息时，前端解除暂停，把“且慢”和新消息一起
形成新的投递轮次。已经交给 CLI 的输入归本机 Agent 会话边界，前端不撤回也不重放。

前端通过 `agentSessionId` 恢复本机 Agent 自己的会话；结构化回合输入承载本轮玩家消息和参与者信息。

玩家也可以通过聊天窗口头部左侧的重置入口手动清空当前可见聊天消息。手动重置会新建本地投递会话、
清空 `agentSessionId` 和等待投递的玩家消息。若手动重置发生在 CLI 调用期间，前端会取消当前调用，避免
旧调用结果写回新会话。
...
产出顺序追加到可见对话记录。玩家发起中断后，“且慢”会与下一条普通玩家消息形成下一次调用；被取消调用
中已交给 CLI 的输入不由前端重放。后续如果需要让 Agent 了解前端侧故障、状态压缩说明或更严格的中间答复
轮次归属，应使用独立的前端事实字段描述这些事件；可见对话记录仍保持为界面层数据。
