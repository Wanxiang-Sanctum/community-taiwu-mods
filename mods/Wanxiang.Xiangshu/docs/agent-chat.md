...
本设计按运行职责划分模块，而不是按当前文件清单描述实现进度：

- `Config.Lua` 暴露本机 Agent 类型、CLI 入口和工作目录设置；这些设置是启动参数，运行时重建依赖游戏重启。
- `src/Frontend/Agent/` 负责 CLI Agent 调用、结构化回合输入、结果解析和临时协议文件。
- `src/Frontend/Chat/` 负责前端投递会话、会话快照、可见消息模型和游戏内聊天窗口。
- `src/Frontend/HotKeys/` 负责热键注册和打开聊天界面所需的游戏状态判断。
- `src/Frontend/Ipc/` 暴露前端侧 IPC 能力，包括前端脚本执行和中间答复写入。
- `src/Frontend/Sidecar/` 负责 MCP sidecar 进程生命周期。
- `src/Backend/` 暴露后端侧 IPC 能力，包括后端脚本执行。
- `src/Ipc/` 定义跨进程 contract、endpoint manifest 和 loopback endpoint 辅助方法。
- `src/Scripting/` 提供前后端复用的受信 C# 编译与执行器。
- `src/McpServer/` 把 MCP 工具调用路由到目标侧 IPC，并整理工具返回。

游戏内入口始终归前端对话会话；工具执行按目标侧 IPC 边界分发。运行时 DLL 归属由项目文件和万象引维护，
不在这份对话设计文档中复制。
...
运行时继续使用本次启动时加载的值，重启游戏后再按新配置重建 IPC endpoint、manifest 路径、MCP
