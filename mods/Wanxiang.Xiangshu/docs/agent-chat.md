...
- `AGENTS.md` 与 `CLAUDE.md` 是对应 CLI Agent 的入口文件。
- `persona/`、`lore/` 和 `tool-guides/` 分别承载玩家可见口吻、静态世界观和运行工具指引。
- `.agents/skills/` 与 `.claude/skills/` 是对应 CLI Agent 自动发现的技能目录。
- `.xiangshu-notes/` 是可选本机工作记录目录，默认包不创建它。
...
相枢身份由默认工作区指令和游戏内显示层固定，不作为投递输入字段投递。前端捕获 Codex `thread_id` 或
Claude Code `session_id` 后，在后续投递轮次通过 CLI resume 参数恢复同一个外部会话。
