...
- 太吾参与战斗时，涉及太吾的运行期功法特殊效果注册判定和属性修正枚举跳过太吾，但普通外伤和身法本体等基础配置计算保留。
...
- 战斗运行期功法特殊效果注册只拦截 `Cast` 和 `EnterCombat`；`Custom`、`Add`、`AddPercent` 和 `TotalPercent` 修改枚举在太吾参战时统一过滤太吾相关的功法特殊效果。这两类入口分别覆盖运行期功法特效注册和数值修正，不依赖战斗状态 `srcCharId` 归因。
...
- 功法特殊效果属性修正集中 patch `SpecialEffectDomain.GetModifyValue`、`GetTotalPercentModifyValue` 和 `CalcCustomModifyEffectList`。同一个规则只过滤 `CombatSkillEffectBase`：目标是战斗中的太吾、功法持有人是战斗中的太吾，或特效持有人是战斗中的太吾时，该功法特效不参与本次修改计算。`GetModifyValue` 和 `GetTotalPercentModifyValue` 只在游戏原本枚举 `AffectDatas.TryGetValue` 前插入过滤，不复制或接管原版求和规则。漂移信号是游戏新增 `EDataModifyType`、新增不经过这些入口枚举 `SpecialEffectList` 的属性修正路径，或这些入口的特效枚举 IL 形态变化导致注入计数失败。
...
- 身法本体提供的移动速度基础配置不是运行时功法特殊效果对象，仍按游戏原本 `AffectingMoveSkillId` 和功法配置计算；身法附带的功法特殊效果若通过 `CombatSkillEffectBase` 修改移动速度、攻速、施展速度或其它字段，则按上面的太吾相关过滤规则跳过。
