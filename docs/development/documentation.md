...
同一事实需要面向多个读者出现时，先判断每个读者需要据此完成什么动作，再按读者重写，不维护一套“对外/对内禁语”。
玩家文案说明可观察行为、获取方式、配置项、存档影响、权限能力、数据留存和信任边界；维护文档说明实现选择、模块归属、
构建/组包、调试路径和设计依据。

不要把“这里不说明内部实现细节”“为避免暴露内部细节”写成对外占位。某个细节不影响玩家安装、使用、迁移、权限或安全判断时，
直接省略或链接最近的维护入口；如果影响这些判断，把它改写为可验证的外部契约，例如本机服务监听范围、token 存放位置、
脚本是否有沙箱、配置是否写入存档、工具返回哪些字段。

“内部”本身不是保密理由。公开源码、模块名、组包 item、设计文档等信息按最近拥有者放入维护文档；凭据、临时 token、
玩家本机设置、对话记录、不应给外部客户端消费的运行态路由等信息按信任边界说明存放位置、可见范围和生命周期，避免用模糊的
内部细节替代具体风险。
...
lock file、解决方案注册和引用方项目文件中的 `ProjectReference`。现有项目以项目文件、`Taiwu.Mod.Pack.proj`、
lock file、目录 README 和解决方案注册为准；`templates/` 只用于创建新项目的初始骨架。
